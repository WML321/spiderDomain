...
class spiderDomain:
    ...
    def help(self):
        print("-h               帮助")
        print("-e--engine       通过引擎(百度)的方式进行搜索，默认是使用字典搜索")
        print("-d               通过字典的形式进行爆破")
        print("--dict            指定字典")
        print("--de              去重")
        print("-o--output          保存至指定文件")
        print("-e--exit          退出")
    def estimate_have_next(self,html, xpath):
        try:
            have_next = html.xpath(xpath)
            if len(have_next) == 0: return 0
            return 1
        except:
            return 0
    ...
    def via_baidu(self):
        # 爬取百度的，就不用什么头部信息了
        url = "https://www.baidu.com/s?wd=inurl:{}&pn={}"
        ...
        header = {
            ...
            # 'x-forwarded-for': '49.4.30.168',
            'Cookie': 'BIDUPSID=FB4C54F83AC905910070E03C58CA624E; PSTM=1593589375; BD_UPN=12314753; __yjs_duid=1_7b3378f06eb2d7d1ad54ade38de3c4921619586754845; BAIDUID=3C411CC6C4472326E7B07ACA6705AAEC:FG=1; BDUSS=FY3Z1FKRXFtYUpYckpXcWZLMzVITkJ2fnRPU1U4blhyT1NGZFdYem1LTWJpR0poRVFBQUFBJCQAAAAAAAAAAAEAAADqH~E0vfDFo1dNTDZkNAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABv7OmEb-zphU; BDUSS_BFESS=FY3Z1FKRXFtYUpYckpXcWZLMzVITkJ2fnRPU1U4blhyT1NGZFdYem1LTWJpR0poRVFBQUFBJCQAAAAAAAAAAAEAAADqH~E0vfDFo1dNTDZkNAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABv7OmEb-zphU; H_WISE_SIDS=107313_110085_127969_164326_168388_177942_178384_178613_179425_179623_181133_181136_181487_181588_181713_181825_182232_182273_182531_182596_182860_183031_183225_183330_183536_183587_183611_183869_183976_184012_184267_184321_184441_184560_184579_184736_184792_184793_184809_184893_185029_185036_185251_185268_185305_185518_185747_185892_185903_185958_186039_186315_186317_186412_186566_186597_186636_186643_186655_186677_186832_186841_187062_187176_187195_187206_187215_187287_187325_187387_187418_187432_187495_187533_187773_187788_187821_187828_8000064_8000109_8000127_8000135_8000150_8000164_8000165_8000167_8000177_8000180_8000185_8000187; MCITY=-%3A; BDORZ=B490B5EBF6F3CD402E515D22BCDA1598; delPer=0; BD_CK_SAM=1; PSINO=1; kleck=5bd9c6b0897f669f81586e11d1ca22a2; sugstore=0; channel=wappass.baidu.com; baikeVisitId=e63d8dbb-18c5-4d6e-b99a-c81e9c8d071f; H_PS_PSSID=35410_35283_35105_31253_35630_35457_34584_35490_34812_35323_26350_35475; H_PS_645EC=3edezdT%2Bux9qcHoF5rQ6XZZv%2FU8G54z6yhtf9LKsURugCLf53VptIfyinjQ; BA_HECTOR=al8020042g008g01vp1gu2qa80r; BDSVRTM=238'
        }
        response = session.get(url.format(domain, 0), headers=header)

        html = etree.HTML(response.content.decode())
        tags = html.xpath("//a[@class='c-showurl c-color-gray']")
        # print(tags)
        address_list = []
        index = 0
        while True:
            index += 1
            for tag in tags:
                address = tag.xpath('.//text()')
                href = tag.xpath('./@href')[0]
                address.append("(href: {})".format(href))
            ...
            if self.estimate_have_next(html=html, xpath="//*[@class='n' and contains(text(),'下一页')]") == 1:
                response = session.get(url.format(domain, index * 10), headers=header)
    ...
    def via_bomb(self,array:list):
        init_domain = self.argvs[0]
        total = len(array)
        index = 1
        for i in array:
            url1 = "http://{0}.{1}".format(i, init_domain)
            url2 = "https://{0}.{1}".format(i, init_domain)
            session = requests.session()
            try:
                response = session.get(url1, headers={
                    'Connection': 'close',
                    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36'
                }, timeout=3)
                if response.status_code >= 200 and response.status_code < 400:
                    temp = "状态码：{}".format(response.status_code)
                    print(url1)
                    print(temp)
                    self.result.append(url1)
                    self.result.append(temp)
                    pass
            except requests.exceptions.RequestException as re:
                pass
            print("进度: {}/{}".format(index, total))
            index += 1
    ...
    def run(self):
        ...
        for opt,value in self.opts:
            ...
            if opt in ['-o','--output']:
                if value=='':
                    self.result_name = './'+str(time.time()).replace('.','')+'.txt'
                else:
        ...
        if utilize:
            ...
            if self.assign_dictionarty_bool:
                self.via_bomb(self.assign_dictionary)
            else:
                self.via_bomb(default_dictionary)
                pass
            pass
        self.save_result(self.result_name)
...
def main(argv):
    try:
        opts,argvs = getopt.getopt(argv,"hedo:i:",["help","output=","exit","dict="])
        # 对于输出参数先进行检查，如果有相互排斥的参数，或者缺少附加参数的应该提出错误
        print(opts)
        print(argvs)
        #
        out_put_flag=0
        ...
        for opt,value in opts:
            if opt=='-output':
                if value == '': out_put_flag=1
            if opt=='-exit': exi_t_flag=1
            if opt=='-dict':
                if value=='': dic_t_flag=2
        if exi_t_flag==1:
            print("输入错误")
            sys.exit(1)
        if out_put_flag==1:
            print("缺少指定的输出路径")
            sys.exit(1)
        if dic_t_flag==e_flag or dic_t_flag==2:
            print("请检查是否同时使用了字典参数和引擎参数或者未设置字典路径")
            sys.exit(1)
        spider = spiderDomain(opts,argvs)
        spider.run()
    except:
        print("发生错误")
        sys.exit(1)
